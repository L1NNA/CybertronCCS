...
def selection(input_length, input_dim, filters, new_len, model_path=None, kernel_size=10):
    ...
    # batch x filters x input_len
    # panalize overlap trunks:
    picked = tf.random.uniform(
        [2], minval=0, maxval=filters, dtype=tf.int64
    )
    kl = (1 - tf.keras.losses.KLDivergence(
        reduction=tf.keras.losses.Reduction.NONE
    )(logits[:, picked[0], :], logits[:, picked[1], :])) * tf.cast(
        picked[0] != picked[1], dtype=tf.float32)
    # batch x 1
    ...
    return Model(l_input, [selected, probs, kl], name='selection')
...
class RLModelKL(BaseModel):
    ...
    def preprocess(self, pairs: tf.data.Dataset, train=False, batch_size=64):
        ...
        pairs = pairs.map(lambda x, y: (x, (y, tf.repeat(y, self.filters + 1, axis=1), 0)))
        return pairs
    ...
    def build(self):
        ...
        # connect together
        source_selected, source_prob, src_kl = l_abstraction(l_embedding(l_source_input))
        dest_selected, dest_prob, des_kl = l_abstraction(l_embedding(l_dest_input))
        ...
        mean_kl = (src_kl + des_kl)/2
        # [batch, 1]

        self._cos_model = Model(
            name=self._name,
            inputs={'org': l_source_input, 'obf': l_dest_input},
            outputs=[predict, maxlikelihood, mean_kl]
        )

        # compile model
        self._cos_model.compile(
            loss=['binary_crossentropy', MaximumLikelihoodLoss(self.filters), 'MSE'],
            optimizer='adam',
            metrics=[['accuracy', 'AUC', 'Precision', 'Recall'], [], []],
            loss_weights=[0.2, 0.8, 0.4]
        )
